...
// import { changeVisibility } from '../../../../32_redux/32-redux/src/store/isVisibleReducer';
...
const counterSlice = createSlice({
    ...
    reducers: {
        plus: (state, action) => {
            state.number += parseInt(action.payload.payload.money); // 상태를 직접 변경 (immer 라이브러리 내부적으로 사용.)
        },
        minus: (state, action) => {
            state.number -= parseInt(action.payload.payload.money);
        }
    },
});
