...
interface StudentInfo {
    ...
    age: number;
    part?: string;
    handleClick: (name: string, age: number) => void;
}

// export default function Student({name, age, part, handleClick}: StudentInfo) {
//     return (
//         <div onClick={() => handleClick(name, age)}>
//             <li>Name: {name}</li>
//             <li>Age: {age}</li>
//             <li>major: {part}</li>
//         </div>
...
// 2. 사용 안한.ver
const Student = ({ name, age, part, handleClick }: StudentInfo) => {
    return (
        <div onClick={() => handleClick(name, age)}>
            <li>이름 : {name}</li>
            <li>학년 : {age}</li>
